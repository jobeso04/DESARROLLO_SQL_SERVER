 ...
 SELECT 'EMERGENCIA' AS SERVICIO, FECHA, B.HISTORIA, A.NOMBRES, CIEX1 AS CIEX FROM EMERGENCIA A LEFT JOIN PACIENTE B ON B.PACIENTE = A.PACIENTE  WHERE FECHA BETWEEN @lcfecha1 AND @lcfecha2 AND substring(A.EDAD,1,3) < '005' and
CIEX1 IN (select id_ciex from @lc_lista_de_ciex) AND TIPO_CIEX1 = 'D' 
  UNION ALL 
  
SELECT 'EMERGENCIA' AS SERVICIO, FECHA, B.HISTORIA, A.NOMBRES, CIEX2 AS CIEX FROM EMERGENCIA A LEFT JOIN PACIENTE B ON B.PACIENTE = A.PACIENTE  WHERE FECHA BETWEEN @lcfecha1 AND @lcfecha2 AND substring(A.EDAD,1,3) < '005' and
CIEX2 IN (select id_ciex from @lc_lista_de_ciex) AND TIPO_CIEX2 = 'D' 
 UNION ALL
 
SELECT 'EMERGENCIA' AS SERVICIO, FECHA, B.HISTORIA, A.NOMBRES, CIEX3 AS CIEX FROM EMERGENCIA A LEFT JOIN PACIENTE B ON B.PACIENTE = A.PACIENTE  WHERE FECHA BETWEEN @lcfecha1 AND @lcfecha2 AND substring(A.EDAD,1,3) < '005' and
