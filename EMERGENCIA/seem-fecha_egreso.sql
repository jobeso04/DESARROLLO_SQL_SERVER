...
/* TRUNCATE TABLE [SIGSALUD].[dbo].[SEEMEMERGENCIA]
...
CASE WHEN CIEX3 = '0' THEN '' ELSE REPLACE(CIEX3,'.', '') END AS CODDIAG3, case when TIPO_CIEX3 in ('P', 'D', 'R') then rtrim(TIPO_CIEX3) when LEN(CIEX3) > 1 then 'D' else '' end  AS TIPDIAG3,
   ...
   CASE WHEN HORA_SAL = '__:__' THEN '' WHEN HORA_SAL IS NULL THEN '' ELSE HORA_SAL END AS HOREGR, case when destino = '01' then '01' when DESTINO = '02' then '02' when DESTINO IN ('04','08', '11') then '03'
          when DESTINO = '06' then '04' when DESTINO = '03' then '05' else '01' end as DESTINO, '' AS DES_EESS, '' DES_UPS, QUIEN_ATIENDE AS CODPSAL, 
          0 AS OBSERV, '' OFECING, '' OHORING, '' OFECEGR, '' OHOREGR,
   ...
   FROM  [SIGSALUD].[dbo].[EMERGENCIA] A   INNER JOIN [SIGSALUD].[dbo].[PACIENTE] B ON B.PACIENTE = A.PACIENTE  INNER JOIN [SIGSALUD].[dbo].[CONSULTORIO] C ON A.CONSULTORIO = C.CONSULTORIO 
    WHERE month(convert(date, fecha_sal)) = @lnmes and year(convert(date, fecha_sal)) = @lnanio   and a.CIEX1 <> '0'  and fecha_sal not in ('__/__/__  ', ' _/__/__  ') and fecha_salida is not null
      order by apell
       ...
       select month(convert(date, fecha_sal)), year(convert(date, fecha_sal)) from EMERGENCIA  where 
          fecha_sal not in ('__/__/__  ', ' _/__/__  ') and fecha_salida is not null and 
         month(convert(date, fecha_sal)) = 1 and year(convert(date, fecha_sal)) = 2018
         */
