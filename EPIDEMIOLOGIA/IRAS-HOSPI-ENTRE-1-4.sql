 /* IRAS HOSPITALIZACION ENTRE 1 Y 4 AÑOS */
 /* 
   ...
   DECLARE @lcfechafin datetime =  CONVERT(DATETIME, '2017-09-09', 101)  
	...
	insert into  @lctablenoti
	 select 'HOSPITALIZACION' AS EMERGENCIA_ID, FECHA1 AS FECHA, NOMBRES  = '', PACIENTE, FECHA_NACIMIENTO = '', CONVERT(VARCHAR(4), EDAD) AS EDAD, SEXO = '',
	  ...
	  where FECHA1 BETWEEN @lcfechainicio AND @lcfechafin and DX1 IN (SELECT DIAG FROM @lcietabla) and TipoEdad IN (@lctipoedad_anios_hospitalizacion) and Edad between @lcedad and @lcedad2
     ...
     INSERT INTO [SIGSALUD].[dbo].[TMP_EPIDEMIO_EDAS]([TITULO],[CDISTRITO],[CLOCALIDAD],[MENOR1])
    	select 'HOSPITALIZACION MENOR ENTRE 1 Y 4 AÑOS ' AS TITULO, (case when b.distrito = 'LURIGANCHO' THEN 'CHOSICA' ELSE b.distrito  END) as cdistrito, upper(c.nombre) as clocalidad, count(a.paciente) as menor1 
