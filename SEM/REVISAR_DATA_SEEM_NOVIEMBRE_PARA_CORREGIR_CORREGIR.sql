...
SELECT motaten  FROM  dbo.SEEMEMERGENCIA  group by motaten  ORDER BY motaten  ASC

update dbo.SEEMEMERGENCIA set MOTATEN = '00' where motaten in ('0', '99')
...
select  * from dbo.SEEMEMERGENCIA order by CONVERT(DATETIME, FECEGR, 3)  asc
use sigsalud	
...
SELECT DOC_IDEN, LEN(DOC_IDEN) AS LONGITUD FROM dbo.SEEMEMERGENCIA where LEN(DOC_IDEN) = 9
UNION ALL 
...
UNION ALL 
SELECT DOC_IDEN, LEN(DOC_IDEN) AS LONGITUD FROM dbo.SEEMEMERGENCIA where LEN(DOC_IDEN) = 1
...
select FECEGR from dbo.SEEMEMERGENCIA  WHERE FECEGR = '' 
...
select * from dbo.SEEMEMERGENCIA order by convertfecate asc