...
/****** Object:  StoredProcedure [dbo].[SP_GET_EMERGENCIA_18_NOMBRE]    Script Date: 01/10/2017 14:23:22 ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[SP_GET_EMERGENCIA_18_NOMBRE](
@p_fecha VARCHAR(10),
...
IF (@abreviatura = 'MED') AND (@p_mispacientes = 1)
	SET @estado = 3
ELSE IF (@abreviatura IN ('ENF', 'TEC')) OR (@p_mispacientes = 0)
BEGIN
	SET @estado = 2
	SET @medico = ''
END
...
AND T1.CONSULTORIO NOT IN ('4060')
...
AND
CASE
WHEN @p_mispacientes = 1 AND @abreviatura = 'MED' THEN T1.QUIEN_ATIENDE
ELSE ''
END = @medico
