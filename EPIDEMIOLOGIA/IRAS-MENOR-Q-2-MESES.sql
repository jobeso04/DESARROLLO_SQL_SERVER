...
/*  DECLARE @lcfechainicio datetime = ?lfechainicio
	...
	SELECT  'EMERGENCIA' EMERGENCIA_ID, fecha, NOMBRES, PACIENTE, FECHA_NACIMIENTO,  EDAD, SEXO, DIRECCION, DISTRITO, LOCALIDAD, CIEX4 as CIEXTOTAL
	 FROM [SIGSALUD].[dbo].[EMERGENCIA] WHERE fecha between @lcfechainicio and @lcfechafin AND CIEX4 IN (SELECT DIAG FROM @lcietabla ) AND convert(int, REPLACE(SUBSTRING(EDAD,1,4),'a',''))  = 0  and convert(int, REPLACE(SUBSTRING(EDAD,5,2),'m','')) < @lcedad
	union all
	SELECT  'EMERGENCIA' EMERGENCIA_ID, fecha, NOMBRES, PACIENTE, FECHA_NACIMIENTO,  EDAD, SEXO, DIRECCION, DISTRITO, LOCALIDAD, CIEX5 as CIEXTOTAL
	 FROM [SIGSALUD].[dbo].[EMERGENCIA] WHERE fecha between @lcfechainicio and @lcfechafin AND CIEX5 IN (SELECT DIAG FROM @lcietabla ) AND convert(int, REPLACE(SUBSTRING(EDAD,1,4),'a',''))  = 0  and convert(int, REPLACE(SUBSTRING(EDAD,5,2),'m','')) < @lcedad
	union all
	SELECT  'EMERGENCIA' EMERGENCIA_ID, fecha, NOMBRES, PACIENTE, FECHA_NACIMIENTO, EDAD,  SEXO, DIRECCION, DISTRITO, LOCALIDAD, CIEX6 as CIEXTOTAL
	 FROM [SIGSALUD].[dbo].[EMERGENCIA] WHERE fecha between @lcfechainicio and @lcfechafin AND CIEX6 IN (SELECT DIAG FROM @lcietabla ) AND convert(int, REPLACE(SUBSTRING(EDAD,1,4),'a',''))  = 0  and convert(int, REPLACE(SUBSTRING(EDAD,5,2),'m','')) < @lcedad
	UNION ALL
	SELECT 'CONSULTORIO' AS EMERGENCIA_ID, FECHA,  PACIENTE AS  NOMBRES, FICHAFAM AS  PACIENTE, FECHA_NACIMIENTO = '', CONVERT(VARCHAR(4), EDAD) AS EDAD, SEXO, 
	       ...
	       WHERE CONVERT(datetime, fecha, 103) BETWEEN @lcfechainicio AND @lcfechafin AND CODIGO1 IN (SELECT DIAG FROM @lcietabla)  and DIAGNOST1 in (select td from @ltipodiagnostico)  AND TIPOEDAD in (@ltipoedad_mes) and EDAD < @lcedad
	UNION ALL
	SELECT 'CONSULTORIO' AS EMERGENCIA_ID, FECHA, PACIENTE AS  NOMBRES, FICHAFAM AS  PACIENTE, FECHA_NACIMIENTO = '', CONVERT(VARCHAR(4), EDAD) AS EDAD, SEXO, 
	       NOMDIST_CITA AS DIRECCION, COD_DIST AS DISTRITO,  LOCALIDAD = '', CODIGO2 AS CIEXTOTAL   FROM [SIGSALUD].[dbo].[HIS]
	       WHERE CONVERT(datetime, fecha, 103) BETWEEN @lcfechainicio AND @lcfechafin AND CODIGO2 IN (SELECT DIAG FROM @lcietabla)  and DIAGNOST2 in (select td from @ltipodiagnostico)  AND TIPOEDAD in (@ltipoedad_mes) and EDAD < @lcedad
	UNION ALL
	SELECT 'CONSULTORIO' AS EMERGENCIA_ID, FECHA, PACIENTE AS  NOMBRES, FICHAFAM AS  PACIENTE, FECHA_NACIMIENTO = '', CONVERT(VARCHAR(4), EDAD) AS EDAD, SEXO, 
	       NOMDIST_CITA AS DIRECCION, COD_DIST AS DISTRITO,  LOCALIDAD = '', CODIGO3 AS CIEXTOTAL   FROM [SIGSALUD].[dbo].[HIS]
	       WHERE CONVERT(datetime, fecha, 103) BETWEEN @lcfechainicio AND @lcfechafin AND CODIGO3 IN (SELECT DIAG FROM @lcietabla)  and DIAGNOST3 in (select td from @ltipodiagnostico)  AND TIPOEDAD in (@ltipoedad_mes) and EDAD < @lcedad
	UNION ALL
	SELECT 'CONSULTORIO' AS EMERGENCIA_ID, FECHA, PACIENTE AS  NOMBRES, FICHAFAM AS  PACIENTE, FECHA_NACIMIENTO = '', CONVERT(VARCHAR(4), EDAD) AS EDAD, SEXO, 
	       NOMDIST_CITA AS DIRECCION, COD_DIST AS DISTRITO,  LOCALIDAD = '', CODIGO4 AS CIEXTOTAL   FROM [SIGSALUD].[dbo].[HIS]
	       WHERE CONVERT(datetime, fecha, 103) BETWEEN @lcfechainicio AND @lcfechafin AND CODIGO4 IN (SELECT DIAG FROM @lcietabla)  and DIAGNOST4 in (select td from @ltipodiagnostico)  AND TIPOEDAD in (@ltipoedad_mes) and EDAD < @lcedad
	UNION ALL
	SELECT 'CONSULTORIO' AS EMERGENCIA_ID, FECHA, PACIENTE AS  NOMBRES, FICHAFAM AS  PACIENTE, FECHA_NACIMIENTO = '', CONVERT(VARCHAR(4), EDAD) AS EDAD, SEXO, 
	       NOMDIST_CITA AS DIRECCION, COD_DIST AS DISTRITO,  LOCALIDAD = '', CODIGO5 AS CIEXTOTAL   FROM [SIGSALUD].[dbo].[HIS]
	       WHERE CONVERT(datetime, fecha, 103) BETWEEN @lcfechainicio AND @lcfechafin AND CODIGO5 IN (SELECT DIAG FROM @lcietabla)  and DIAGNOST5 in (select td from @ltipodiagnostico)  AND TIPOEDAD in (@ltipoedad_mes) and EDAD < @lcedad
	UNION ALL
	SELECT 'CONSULTORIO' AS EMERGENCIA_ID, FECHA, PACIENTE AS  NOMBRES, FICHAFAM AS  PACIENTE, FECHA_NACIMIENTO = '', CONVERT(VARCHAR(4), EDAD) AS EDAD, SEXO, 
	       NOMDIST_CITA AS DIRECCION, COD_DIST AS DISTRITO,  LOCALIDAD = '', CODIGO6 AS CIEXTOTAL   FROM [SIGSALUD].[dbo].[HIS]
	       WHERE CONVERT(datetime, fecha, 103) BETWEEN @lcfechainicio AND @lcfechafin AND CODIGO6 IN (SELECT DIAG FROM @lcietabla)  and DIAGNOST6 in (select td from @ltipodiagnostico)  AND TIPOEDAD in (@ltipoedad_mes) and EDAD < @lcedad
	UNION ALL
	 select 'HOSPITALIZACION' AS EMERGENCIA_ID, FECHA1 AS FECHA, NOMBRES  = '', PACIENTE, FECHA_NACIMIENTO = '', CONVERT(VARCHAR(4), EDAD) AS EDAD, SEXO = '',
	  ...
	  where FECHA1 BETWEEN @lcfechainicio AND @lcfechafin and DX1 IN (SELECT DIAG FROM @lcietabla) and TipoEdad = @ltipoedad_mes_hospitalizacion and Edad < @lcedad
