...
/*  declare @lcfecha1 datetime = convert(datetime, ?lcfecha, 101)
  ...
  declare @lc_lista_de_ciex  table (id_ciex varchar(20)); insert @lc_lista_de_ciex(id_ciex)  values('J01')
  ...
  AND FECHA BETWEEN @lcfecha1 AND @lcfecha2) AND TIPODX = 'D' and DX LIKE (SELECT ID_CIEX FROM @lc_lista_de_ciex) + '%' 
  group by dx
...
/* declare @lcfecha1 datetime = convert(datetime, ?lcfecha, 101)
  ...
  select NOMCONSULTORIO AS SERVICIO, FECHA, A.HISTORIA, A.NOMBRES, C.DX AS CIEX from V_ATENCIONC A  LEFT JOIN 
   PACIENTE B  ON B.PACIENTE = A.PACIENTE  LEFT JOIN ATENCIOND C ON C.ID_CITA = A.ID_CITA
    ...
    AND C.TIPODX = 'D' AND C.DX LIKE (SELECT ID_CIEX FROM @lc_lista_de_ciex) + '%' 
