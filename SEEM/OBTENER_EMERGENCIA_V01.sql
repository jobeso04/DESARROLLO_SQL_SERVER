...
SELECT [IDSEEMEMERGENCIA],[RENIPRESS],[E_UBIG],[COD_DISA],[COD_RED],[COD_MRED],[fecate],[horate],[numhc],[doc_iden],[etnia],[FINANCIA],[SEXO],[EDAD],[tipoedad],[NOMB],[APELL],
  [DIRECC],[UBIG_RESHA],[UBIG_PROCE],[ACOMPANA],[ADOC_IDEN],[MOTATEN],[SITOCURREN],[UPS],[CODDIAG1],[TIPDIAG1],[CODDIAG2],[TIPDIAG2],[CODDIAG3],[TIPDIAG3],[CODDIAG4],[TIPDIAG4],[codcpt1],
  [codcpt2],[codcpt3],[codcpt4],[CONDICION],[FECEGR],[HOREGRE],[DESTINO],[DES_EESS],[DES_UPS],[CODPSAL],[OBSERV],[OFECING],[OHORING],[OFECEGRE],[TOTALEST],[OCAMA],[OCODPSAL],
  [OCODDIAG1],[OCODDIAG2],[FECHAREG],[ESTADO]
  FROM [SIGSALUD].[dbo].[SEEMEMERGENCIA]
...
update EMERGENCIA set SEGURO = '0' where SEGURO IN ('}', 'Ç') 
UPDATE PACIENTE SET DOCUMENTO = '0' WHERE DOCUMENTO IN ('R.N.', '01', 'NO RECUERDA', 'NO TIENE', 'NO SABE', '0', 'NO REFIERE', 'EN TRAMITE', 'NO SE ACUERDA', 'NO TRAJO', '00000000', 'TRAMITE', 'NO LO TRAE', 'NO REGISTRA',
     '000000000', '000', '.07656544',  ' RUN 23512831-4',  '             NO SABE', ' NO RECUERDA', ' NO SABE', ' NO SE ACUERDA', ' NO TIENE', '.', 'NO SE RECUERDA', 'R.N', 'RECUERDA', 'NO REDUERDA', 'NO RECUEDA', 'NO RECDUERDA', 'NO RECUERDO', 'NO RECFUERDA', 'NO TIENE DNI',
      'NOTIENE', '}', 'Ç', 'NO TRAE', 'NOSABE', '0000000',  'NO LO TIENE', '3', 'NO RECUERSA', '07', 'NO HAY SISTEMA', 'NO RECUERA', '99999999', 'NO RECUERDA LA MADRE' )
...
update EMERGENCIA set DOCUMENTOA = '0' where DOCUMENTOA IN ('NORECUERDA', '', 'NO RECUERDA', 'NO REFIERE', 'no recuerda', 'NO SABE', 'NO RECUERDA', 'NO TIENE', '--', '---------------------', '-----------------------', '.',
  ' NO SE ACUERDA', ' en tramite', '}', '}}', '+', '0', '99999999') 
  UPDATE EMERGENCIA SET CONDICION_EGRESO = '0' WHERE CONDICION_EGRESO = ''
  update EMERGENCIA set DESTINO = '0' where DESTINO = ''  */
  
  
     SELECT IDSEEMEMERGENCIA, RTRIM(coddiag3) DIA2  FROM dbo.SEEMEMERGENCIA WHERE LEN(coddiag3) = 3 
          SELECT IDSEEMEMERGENCIA,case when FECEGR = '' then '' when substring(FECEGR,3,1) = '/' then substring(FECEGR,1,6) + substring(FECEGR,9,2)
              when substring(FECEGR,5,1) = '-' then substring(FECEGR,9,2) + '/' + substring(FECEGR,6,2) + '/' + substring(FECEGR,3,2)  end as fec  FROM dbo.SEEMEMERGENCIA