/* trama b1 */
TRUNCATE TABLE [SIGSALUD].[dbo].[TMP_TRAMAB1_ATM]
...
insert @lcatencion_medica values ('TRAUMATOLOGIA'), ('DISPLASIA'), ('NEUROLOGIA'), ('GINECO-OBSTETRICIA'), ('NEUMOLOGIA'), ('MEDICINA INTERNA'), ('OFTALMOLOGIA'), ('DERMATOLOGIA'),
('ENFERMEDADES INFECCIOSAS INFECTOLOGIA'), ('CIRUGIA GENERAL'), ('PEDIATRIA'), ('ENDOCRINOLOGÍA'), ('CIRUGIA RECONSTRUCTIVA'), ('PSIQUIATRIA'), ('GASTROENTEROLOGIA'),
('CARDIOLOGIA')
DECLARE @lctabla_atm_medica table  (sexo varchar(1), edad varchar(3), grupo_edad varchar(2), atencion_medica varchar(4))
DECLARE @lctabla_atm_medica_final table  (sexo varchar(1), edad varchar(3), grupo_edad varchar(2), atencion_medica varchar(4))
...
when tipoedad in ('A') and edad between 60 and 64  then '14' when tipoedad in ('A') and edad >= 65  then '15' end as GRUPO_EDAD, CASE WHEN HSERVICIO IN (select atnmedica from @lcatencion_medica) THEN 1 ELSE 0 END AS ATENCION_MEDICA
from [SIGSALUD].[dbo].[HIS] where PERIODO = @lcperiodo  and edad <> 0 ORDER BY SEXO, GRUPO_EDAD
select sexo, grupo_edad, sum(convert(int, atencion_medica)) as atencion_medica, 0 as ATENDIDOS_MES 
from @lctabla_atm_medica WHERE atencion_medica <> 0  group by sexo, grupo_edad order by sexo, convert(int, grupo_edad)
